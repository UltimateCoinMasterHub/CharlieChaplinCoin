...
#else
    const char* pszModule = "CharlieChaplinCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CharlieChaplinCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CharlieChaplinCoin
    // Mac: ~/Library/Application Support/CharlieChaplinCoin
    // Unix: ~/.CharlieChaplinCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CharlieChaplinCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "CharlieChaplinCoin";
#else
    // Unix
    return pathRet / ".CharlieChaplinCoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "CharlieChaplinCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "CharlieChaplinCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong CharlieChaplinCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("CharlieChaplinCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
