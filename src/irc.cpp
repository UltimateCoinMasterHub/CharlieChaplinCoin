...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("CharlieChaplinCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CharlieChaplinCoinTEST2\r");
            Send(hSocket, "WHO #CharlieChaplinCoinTEST2\r");
        } else {
            // randomly join #CharlieChaplinCoin00-#CharlieChaplinCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #CharlieChaplinCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #CharlieChaplinCoin%02d\r", channel_number).c_str());
        }
