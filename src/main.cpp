...
unsigned int nStakeTargetSpacing = 6.67 * 60; // 6.67-minute stakes spacing
int64_t nTargetSpacing = 6.67 * 60;  // Same as the above
...
const string strMessageMagic = "CharlieChaplinCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(unsigned int nBits, int64_t nFees, int nHeight)
{
	...
	if (nHeight == 1)
		return 877000000 * COIN;
    
	// Force block reward to zero when right shift is undefined.
    int halvings = nHeight / 2200000;
    if (halvings >= 64)
        return nFees;
...
static const int64_t nTargetTimespan = 6.67 * 15 * 60;
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("CharlieChaplinCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in CharlieChaplinCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // CharlieChaplinCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CharlieChaplinCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "ccc genesis";
        CTransaction txNew;
        txNew.nTime = 1488232412;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1488232412;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1864430; // !fTestNet ? 1575379 : 46534;
...
unsigned char pchMessageStart[4] = { 0xb4, 0xf5, 0xe4, 0xe5 };
